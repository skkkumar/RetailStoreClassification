...
int main1(int argc, char* argv[]) {
	...
	for (;;) {
		...
		if (mode == CALIBRATED && s.showUndistorsed) {
			Mat temp = view.clone();
			if (s.useFisheye)
				cv::fisheye::undistortImage(temp, view, cameraMatrix,
						distCoeffs);
			else
	...
	if (s.inputType == Settings::IMAGE_LIST && s.showUndistorsed) {
		...
		cout << cameraMatrix << endl<<flush;
		cout << distCoeffs << endl<<flush;
		if (s.useFisheye) {
			Mat newCamMat;
...
static double computeReprojectionErrors(
		...
		const Mat& distCoeffs, vector<float>& perViewErrors, bool fisheye) {
...
static void calcBoardCornerPositions(Size boardSize, float squareSize,
	...
	case Settings::ASYMMETRIC_CIRCLES_GRID:
		for (int i = 0; i < boardSize.height; i++)
			for (int j = 0; j < boardSize.width; j++)
				corners.push_back(
						Point3f((2 * j + i % 2) * squareSize, i * squareSize,
								0));
		break;
	default:
		break;
